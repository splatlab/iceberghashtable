...
void
ycsb_load_run_randint(int                    index_type,
                      int                    wl,
                      int                    kt,
                      int                    ap,
                      int                    num_thread,
                      ...
                      std::vector<int>      &ops)
{
   ...
   if (ap == UNIFORM) {
      if (kt == RANDINT_KEY && wl == WORKLOAD_A) {
         init_file =
            "/mnt/nvme3/RECIPE/index-microbench/workloads/loada_unif_int.dat";
         txn_file =
            "/mnt/nvme3/RECIPE/index-microbench/workloads/txnsa_unif_int.dat";
      } else if (kt == RANDINT_KEY && wl == WORKLOAD_B) {
         init_file =
            "/mnt/nvme3/RECIPE/index-microbench/workloads/loadb_unif_int.dat";
         txn_file =
            "/mnt/nvme3/RECIPE/index-microbench/workloads/txnsb_unif_int.dat";
      } else if (kt == RANDINT_KEY && wl == WORKLOAD_C) {
         init_file =
            "/mnt/nvme3/RECIPE/index-microbench/workloads/loadc_unif_int.dat";
         txn_file =
            "/mnt/nvme3/RECIPE/index-microbench/workloads/txnsc_unif_int.dat";
      } else if (kt == RANDINT_KEY && wl == WORKLOAD_D) {
         init_file =
            "/mnt/nvme3/RECIPE/index-microbench/workloads/loadd_unif_int.dat";
         txn_file =
            "/mnt/nvme3/RECIPE/index-microbench/workloads/txnsd_unif_int.dat";
      } else if (kt == RANDINT_KEY && wl == WORKLOAD_E) {
         init_file =
            "/mnt/nvme3/RECIPE/index-microbench/workloads/loade_unif_int.dat";
         txn_file =
            "/mnt/nvme3/RECIPE/index-microbench/workloads/txnse_unif_int.dat";
      }
   } else {
      if (kt == RANDINT_KEY && wl == WORKLOAD_A) {
         init_file =
   ...
   txn_count--;
   fprintf(stderr, "Loaded %lu txn keys\n", txn_count);
   ...
   if (index_type == TYPE_ICEBERG) {
      ...
      iceberg_init(&hashtable, 24, 27, 1);
...

      {
         ...
         auto func = [&]() {
            ...
            for (uint64_t i = start_key; i < end_key; i++) {
               if (!iceberg_insert(tds[thread_id].ht,
                                   init_keys[i],
                                   init_keys[i],
                                   thread_id)) {
...

      {
         ...
         auto func = [&]() {
            ...
            for (uint64_t i = start_key; i < end_key; i++) {
               if (ops[i] == OP_INSERT) {
                  if (!iceberg_insert(
                         tds[thread_id].ht, keys[i], keys[i], thread_id)) {
                     printf("Failed insert\n");
                     exit(0);
                  }
                  ...
                  uintptr_t *val;
                  auto       ret =
                     iceberg_get_value(tds[thread_id].ht, keys[i], &val);
                  if (*val != keys[i]) {
...
main(int argc, char **argv)
{
   ...
   if (kt != STRING_KEY) {
      ...
      ycsb_load_run_randint(
         index_type, wl, kt, ap, num_thread, init_keys, keys, ranges, ops);
   }
   /*
   else {
       ...
       ycsb_load_run_string(index_type, wl, kt, ap, num_thread, init_keys, keys,
   ranges, ops);
   }
   */
