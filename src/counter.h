...
typedef __attribute__((aligned(64))) struct counter {
  cache_aligned_counter global[8];
  cache_aligned_counter local_counters[NUM_COUNTERS];
} counter;
...
counter_increment(counter *cntr, uint64_t batch, uint64_t tid)
{
  ...
  if (local == THRESHOLD) {
    ...
    __atomic_fetch_add(
      &cntr->global[batch].count[0], THRESHOLD, __ATOMIC_SEQ_CST);
  }
}
...
counter_sync(counter *cntr, uint64_t batch)
{
  for (uint32_t tid = 0; tid < NUM_COUNTERS; tid++) {
    ...
    cntr->local_counters[tid].count[batch] = 0;
    __atomic_fetch_add(
      &cntr->global[batch].count[0], local_count, __ATOMIC_SEQ_CST);
  }
}
...
{
  return cntr->global[batch].count[0];
}

static inline int64_t
counter_local_sum(counter *cntr,
