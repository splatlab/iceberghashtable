...
static inline uint64_t
truncate_to_current_num_raw_blocks(uint64_t table_log_num_blocks,
                                   uint64_t raw_block)
{
  return raw_block & ((1 << table_log_num_blocks) - 1);
}
                       ...
                       uint64_t         tid)
{
  ...
  return level1_maybe_query_old_block(table, key, value, h, pb) ||
         level1_query_block(table, key, value, h, pb) ||
         level2_query(table, key, value, h) ||
